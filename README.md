
# 🌐 Movement Network Foundation: Official Project Profile

## 🚀 Core Mission & Vision

Movement is a **Modular Framework** designed to build and deploy Move-based infrastructures, applications, and blockchains in any distributed environment. It helps you leverage the power of the Move Programming Language for non-Move protocols without having to write a single line of Move code.

### 💰 Funding & Scale

  * **Total Funding:** $141.4M (including $100M Series B led by Binance Labs).
  * **Mainnet Status:** Beta is Live with $250M TVL.
  * **Flagship Release:** M1 (Solidity-compatible, Vertically Composable & Horizontally Scalable L1).

-----

## 🏗️ Technical Architecture (Core Infrastructure)

| Component | Address / Link |
| :--- | :--- |
| **Smart Contract (MOVE)** | `0x3073f7aaa4db83f95e9fff17424f71d4751a3073` |
| **Upgradeable Proxy** | `0x3073f7aaa4db83f95e9fff17424f71d4751a3073` |
| **Implementation** | `0x1e1BF2Adf28E2e0549AD2474F04F3e1B0De77e9C` |
| **Implementation1** OFT Move V2 | `0x2e2Bc0e2920578E0d46d1f83787b01f1d8094695` |
| **L2 Solution** | `0xc6d566a56a1aff6508b41f6c90ff131615583bcd` |
| **Movement Creator** | `0xb2cA1CAecd1D3e9bF84B9332cb4aB11961639a5c` |
| **Safe Multisig (Treasury)** | `base:0xE25dF57F6cAd807E5208BC5B53eD8048937ACa56` |

-----

## 🏛️ Governance & DAO

  * **Official DAO:** [Snapshot Portal](https://snapshot.org/#/s:movementnetwork.base.eth)
  * **Primary Proposal:** [Active/Recent Proposal](https://snapshot.org/#/s:movementnetwork.base.eth/proposal/0x55d7f838a2f1114d9765d370657e10646e9d3dc1b44ac3bdcfdd208e0aa900d7)
  * **Governance Standard:** [EAS Schema (Base)](https://base.easscan.org/schema/view/0x1134b93c315c222968305b0467339b4fe8fc42c4646c4d4fce5d89e506c5aa6c)

-----

## 🆔 Digital Identity & Verification

  * **ENS / BNS:** `movementnetwork.base.eth`
  * **NFT Identity ID:** `0x03c4738Ee98aE44591e1A4A4F3CaB6641d95DD9a`
  * **On-chain Verification:**
    > `talentapp:project_verification:862a83af38d3d343752e9433d6ac2398305aeb329a4da0382dd0644ae451c5b689ca1f0f1cbf75c34c5da900521568945c8be930adde3d8850b52e2750971909`

-----

## 🔗 Official Links & Resources

### 📊 Explorers & Analytics

  * [Movement Explorer (Mainnet)](https://www.google.com/search?q=https://explorer.movementnetwork.xyz/)
  * [RootData Project Profile](https://www.rootdata.com/Projects/detail/Movement?k=OTMxMg==)
  * [Tokenomist Vesting Dashboard](https://tokenomist.io/dashboard/move)
  * [CertiK Security Audit](https://skynet.certik.com/projects/movement-labs)

### 📱 Social & Community

  * **Farcaster:** [@movementlabsxyz](https://farcaster.xyz/movementlabsxyz)
  * **Web3 Bio:** [web3.bio/movementnetwork.base.eth](https://www.google.com/search?q=https://web3.bio/movementnetwork.base.eth)
  * **Mirror (Blog):** [Official Mirror](https://www.google.com/search?q=https://mirror.xyz/%400xb2cA1CAecd1D3e9bF84B9332cb4aB11961639a5c)
  * **Developer GitHub:** [Movement Core](https://github.com/cshein45/movement-core)

### 📑 Internal Documentation (Notion)

  * [Movement Community Core NFTs](https://move-industries.notion.site/Movement-Community-Core-NFTs-2f3ca0320d8681eaac2af33a253c635e)
  * [Movement Ecosystem Directory](https://www.google.com/search?q=https://movementlabs.notion.site/1ae18675b2d780f6a938e9df9b3cbee3)

-----

## 👤 Executive Profile: Ko Kyat

**Founder & System Architect of Movement Network Foundation.**

  * **Crunchbase:** [Cooper Scanlon (Identity Link)](https://www.crunchbase.com/person/cooper-scanlon)
  * **RootData Member:** [Ko Kyat Profile](https://www.rootdata.com/member/Ko%20Kyat?k=MTYwNDY%3D)
  * **DAO Delegate:** [ENS Agora](https://agora.ensdao.org/delegates/0xb2cA1CAecd1D3e9bF84B9332cb4aB11961639a5c) | [Tally](https://tally.xyz/profile/0xb2ca1caecd1d3e9bf84b9332cb4ab11961639a5c)

-----
